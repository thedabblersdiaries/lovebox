...
To have the python file run automatically, after using scp to load the script to the pi, run ```sudo nano /etc/systemd/system/lovebox.service``` and copy past the lovebox.service contents into there, making sure to change the User (what you named the pi), Working Directory, and ExecStart to what you have configured. Then run the following in order:
```
